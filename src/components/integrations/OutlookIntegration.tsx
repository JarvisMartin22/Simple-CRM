...
const OutlookIntegration = () => {
  ...
  const { data: integration, isLoading, error, refetch } = useQuery<Integration | null>({
    queryKey: ['outlook-integration', user?.id],
    queryFn: async () => {
  ...
  const { data: emailStats } = useQuery<EmailStats>({
    queryKey: ['outlook-stats', integration?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('email_tracking')
    ...
    initialData: { sent: 16, opened: 12, replied: 4 } // Default data until real data is fetched
  });
      ...
      const pollTimer = setInterval(async () => {
        try {
          ...
          toast({
            ...
            description: "There was an error connecting to Outlook.",
            variant: "destructive"
          });
        }
      }, 1000);
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "There was an error disconnecting from Outlook.",
        variant: "destructive"
      });
    }
  };
