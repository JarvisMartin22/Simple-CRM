...
import { CompanyField, CompanyFieldType, SelectOption, useCompanies } from '@/contexts/CompaniesContext';

// Field types available for selection
const fieldTypes: { value: CompanyFieldType; label: string }[] = [
...
interface CompanyFieldEditDialogProps {
  field?: CompanyField;
  mode: 'create' | 'edit';
  onClose?: () => void;
}

export const CompanyFieldEditDialog: React.FC<CompanyFieldEditDialogProps> = ({ field, mode, onClose }) => {
  const { addField, updateField } = useCompanies();
  
  // State for field properties
  const [name, setName] = useState(field?.name || '');
  const [type, setType] = useState<CompanyFieldType>(field?.type || 'text');
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      <div className="space-y-2">
        ...
        <Select
          value={type}
          onValueChange={(value) => setType(value as CompanyFieldType)}
          disabled={field?.id === 'company_name'} // Disable type change for company name field
        >
          <SelectTrigger>
