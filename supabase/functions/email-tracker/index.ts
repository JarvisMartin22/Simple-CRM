...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 DEBUG: Processing email open tracking:`, {
      trackingId,
    ...
    if (!trackingId) {
      console.warn("⚠️ Missing tracking ID in request");
    ...
    console.log(`🔍 DEBUG: Looking up tracking record for ID: ${trackingId}`);
    ...
    console.log(`✅ DEBUG: Found tracking record:`, trackingData);
    ...
    if (updateError) {
      ...
      if (eventError) {
        console.error("⚠️ Error recording event:", eventError);
      }
    }
